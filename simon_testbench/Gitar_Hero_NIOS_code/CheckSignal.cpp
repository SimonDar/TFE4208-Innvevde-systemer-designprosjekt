...
int CheckClose(int InputFrequency) {
    ...
    {
        if (arrayLengt <= halfWay(gitarFrequency_mHz[i], gitarFrequency_mHz[i+1]))
        {
            result = 0;
        }
        else if(i == arrayLengt()-1 && halfWay(gitarFrequency_mHz[i], gitarFrequency_mHz[i+1]) < InputFrequency)
        {
